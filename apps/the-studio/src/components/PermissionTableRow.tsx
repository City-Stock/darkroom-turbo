...
const PermissionTableRow: FC<Props> = ({ value, description, uid }) => {
  ...
  const deletePermission = async (uid: string) => {
    try {
      await toast.promise(onDelete(uid), {
        loading: "Deleting Permission...",
        success: () => {
          return <div>{`Permission deleted successfully`}</div>;
        },
        error: ({ message }) => {
    ...
    <tr>
      ...
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        <div className="flex items-center space-x-3.5 justify-center">
          {isDeleting ? (
            <LoadingSpinner message="" color="white" />
          ) : (
            <DeleteModal
              title={value}
              collectionName="Permission"
              handleDelete={() => deletePermission(uid)}
              icon={deleteIcon}
            />
          )}
        </div>
